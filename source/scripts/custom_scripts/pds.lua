...
function updatePDS(CustomGroup, playerID, shipID)
	...
	if
		SobGroup_AreAnyFromTheseAttackFamilies(CustomGroup, "SmallCapitalShip, BigCapitalShip, Mothership") == 1
		and Player_HasResearch(playerID, "CapitalPDS") == 1
	then
	...
	elseif
		SobGroup_AreAnyFromTheseAttackFamilies(CustomGroup, "Frigate, Capturer") == 1
		and Player_HasResearch(playerID, "CruiserPDS") == 1
	then
