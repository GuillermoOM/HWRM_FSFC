...
function Do_Afterburner(CustomGroup, playerIndex, shipID)
	...
	if
		...
		or CustomGroup == "shi_nahema"
	then
		ABspeed = ABspeed * (4 / 3)
	end
	if
		(
			Player_HasResearch(playerIndex, "SethAfterburner") == 1
			or Player_HasResearch(playerIndex, "SethAfterburnerFS1") == 1
		) and (CustomGroup == "vas_seth" or CustomGroup == "hol_seth")
	then
		ABspeed = ABspeed * (4 / 3)
	end
	--here we can put in afterburner recharge modifiers.
	if
		(SobGroup_IsDoingAbility(CustomGroup, AB_Move) == 1)
		or (SobGroup_IsDoingAbility(CustomGroup, AB_Attack) == 1)
		or (SobGroup_IsDoingAbility(CustomGroup, AB_Dock) == 1)
		or (SobGroup_IsDoingAbility(CustomGroup, AB_Guard) == 1)
	then
