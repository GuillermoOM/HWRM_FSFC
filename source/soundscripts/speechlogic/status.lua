...
function ChatterMiscHighPriority(shipname, parm2, code)
	...
	if code == CD_CapturedAtUnitCap then
		if shipname == "Hgn_MarineFrigate" then
			...
			playSpeechActor(
				"STATUS_INFILTRATORFRIGATE_SUCCESS_CAPFULL",
	...
	elseif code == CD_CaptureHalfway then
		if shipname == "Hgn_MarineFrigate" then
			...
			playSpeechActor(
				"CHATTER_INFILTRATORFRIGATE_50PERCENT",
				NameInfiltratorPilot,
				NumInfiltratorPilots,
				Frequency_Command
			)
	...
	elseif code == CD_CaptureStarted then
		if shipname == "Hgn_MarineFrigate" then
			...
			playSpeechActor(
				"CHATTER_INFILTRATORFRIGATE_BEGIN",
				NameInfiltratorPilot,
				NumInfiltratorPilots,
				Frequency_Command
			)
...
function StatusUnderAttack(shipname, enemy, attackcode)
	...
	if attackMap["" .. shipname] ~= nil then
		playSpeechActor(
			attackMap["" .. shipname][1],
			attackMap["" .. shipname][2],
			attackMap["" .. shipname][3],
			Frequency_Status
		)
		return
	end
	...
	if currentRace >= 10 then
		...
		elseif shiptype == Resource then
			playSpeechActor(
				"STATUS_ResourcePocketEnemiesDetected_1",
				NameSupportPilot,
				NumSupportPilots,
				Frequency_Status
			)
...
function StatusRetaliating(retaliatingShipName, currentTactics)
	...
	if currentRace >= 10 then
		...
		if currentTactics == AggressiveTactics then
			playSpeechActor(
				"STATUS_GenericGroupUnderAttackRetaliate_Aggressive",
				NameAllPilot,
				NumAllPilots,
				Frequency_Status
			)
		elseif currentTactics == DefensiveTactics then
			playSpeechActor(
				"STATUS_GenericGroupUnderAttackRetaliate_Defensive",
				NameAllPilot,
				NumAllPilots,
				Frequency_Status
			)
		elseif currentTactics == PassiveTactics then
			playSpeechActor(
				"STATUS_GenericGroupUnderAttackRetaliate_Passive",
				NameAllPilot,
				NumAllPilots,
				Frequency_Status
			)
...
function StatusUnderFireNeedOrders(underFireShipName, targetShipName, groupNumber)
	...
	if currentRace >= 10 then
		...
		if groupNumber == -1 then
			...
			playSpeechActor(
				"STATUS_AssignedGroupUnderAttack_Report_G" .. groupNumber,
				raceHelper(),
				0,
				Frequency_Status
			)
	...
	if cameradistance > MAX_CAMERA then
		...
		elseif generictargetnm == "Battlecruiser" then
			...
			elseif currentRace == Vaygr then
				playSpeechActor(
					"STATUS_InfiltratorFrigate_Success",
					NameInfiltratorPilot,
					NumInfiltratorPilots,
					Frequency_Status
				)
	...
	if currentRace >= 10 then
		...
		elseif shiptype == Frigate or shiptype == Capital then
			...
			playSpeechActorLinked(
				"CHATTER_CapitalShipSmallHits_Query",
				raceHelper(),
				0,
...
function StatusResources(shipname, actionType)
	...
	if currentRace >= 10 then
		...
		if actionType == 1 then
			playSpeechActor(
				"STATUS_RESOURCEPOCKETEXHAUSTEDGOINGTONEXTPOCKET_1",
				NameSupportPilot,
				NumSupportPilots,
				Frequency_Status
			)
		...
		elseif actionType == 3 then
			playSpeechActor(
				"STATUS_RESOURCEPOCKETEXHAUSTEDGOINGIDLE_1",
				NameSupportPilot,
				NumSupportPilots,
				Frequency_Status
			)
...
function StatusBattleMomentum(momentum)
	...
	if momentum > 5.0 then
		playSpeechActorLinked(
			"CHATTER_GenericGroupStartBattleAdvantaged_Report",
			NameAllPilot,
			NumAllPilots,
			Frequency_Status,
			CE_CHT_ADVRESPONSE
	...
	elseif momentum > 4.0 then
		playSpeechActorLinked(
			"CHATTER_GenericGroupWinningBattle_Report",
			NameAllPilot,
			NumAllPilots,
			Frequency_Status,
			CE_CHT_WINRESPONSE
	...
	elseif momentum > 0.125 then
		playSpeechActorLinked(
			"CHATTER_GenericGroupStartBattleFair_Report",
			NameAllPilot,
			NumAllPilots,
			Frequency_Status,
			CE_CHT_FAIRRESPONSE
	...
	elseif momentum < 0.25 then
		playSpeechActorLinked(
			"CHATTER_GenericGroupLosingBattle_Report",
			NameAllPilot,
			NumAllPilots,
			Frequency_Status,
			CE_CHT_LOSERESPONSE
...
ssMap["Battlecruiser"].BATTLECRUISERENGINE = {
	"STATUS_BATTLECRUISER_EnginesUnderAttack_1",
	"STATUS_BATTLECRUISER_EnginesDamaged_1",
	"STATUS_BATTLECRUISER_EnginesDisabled_1",
}
ssMap["Battlecruiser"].C_MODULE_CLOAKGENERATOR = {
	"STATUS_BATTLECRUISER_CloakGeneratorUnderAttack_1",
	"STATUS_BATTLECRUISER_CloakGeneratorDamaged_1",
	"STATUS_BATTLECRUISER_CloakGeneratorDestroyed_1",
}
ssMap["Battlecruiser"].C_MODULE_FIRECONTROL = {
	"STATUS_BATTLECRUISER_FireControlTowerUnderAttack_1",
	"STATUS_BATTLECRUISER_FireControlTowerDamaged_1",
	"STATUS_BATTLECRUISER_FireControlTowerDestroyed_1",
}
ssMap["Battlecruiser"].C_MODULE_HYPERSPACE = {
	"STATUS_BATTLECRUISER_HyperspaceCoreUnderAttack_1",
	"STATUS_BATTLECRUISER_HyperspaceCoreDamaged_1",
	"STATUS_BATTLECRUISER_HyperspaceCoreDestroyed_1",
}
ssMap["Battlecruiser"].C_MODULE_RESEARCH = {
	"STATUS_BATTLECRUISER_ResearchModuleUnderAttack_1",
	"STATUS_BATTLECRUISER_ResearchModuleDamaged_1",
	"STATUS_BATTLECRUISER_ResearchModuleDestroyed_1",
}
...
ssMap["Carrier"].C_MODULE_CLOAKGENERATOR = {
	"STATUS_CARRIER_CloakGeneratorUnderAttack_1",
	"STATUS_CARRIER_CloakGeneratorDamaged_1",
	"STATUS_CARRIER_CloakGeneratorDestroyed_1",
}
ssMap["Carrier"].C_MODULE_FIRECONTROL = {
	"STATUS_CARRIER_FireControlTowerUnderAttack_1",
	"STATUS_CARRIER_FireControlTowerDamaged_1",
	"STATUS_CARRIER_FireControlTowerDestroyed_1",
}
ssMap["Carrier"].C_MODULE_HYPERSPACE = {
	"STATUS_CARRIER_HyperspaceCoreUnderAttack_1",
	"STATUS_CARRIER_HyperspaceCoreDamaged_1",
	"STATUS_CARRIER_HyperspaceCoreDestroyed_1",
}
ssMap["Carrier"].C_MODULE_PLATFORMCONTROL = {
	...
	"STATUS_CARRIER_PlatformFacilityDestroyed_1",
}
ssMap["Carrier"].C_MODULE_RESEARCH = {
	"STATUS_CARRIER_ResearchModuleUnderAttack_1",
	"STATUS_CARRIER_ResearchModuleDamaged_1",
	"STATUS_CARRIER_ResearchModuleDestroyed_1",
}
ssMap["Carrier"].C_PRODUCTION_CORVETTE = {
	"STATUS_CARRIER_CorvetteFacilityUnderAttack_1",
	"STATUS_CARRIER_CorvetteFacilityDamaged_1",
	"STATUS_CARRIER_CorvetteFacilityDestroyed_1",
}
ssMap["Carrier"].C_PRODUCTION_FIGHTER = {
	"STATUS_CARRIER_FighterFacilityUnderAttack_1",
	"STATUS_CARRIER_FighterFacilityDamaged_1",
	"STATUS_CARRIER_FighterFacilityDestroyed_1",
}
ssMap["Carrier"].C_PRODUCTION_FRIGATE = {
	"STATUS_CARRIER_FrigateFacilityUnderAttack_1",
	"STATUS_CARRIER_FrigateFacilityDamaged_1",
	"STATUS_CARRIER_FrigateFacilityDestroyed_1",
}
ssMap["Carrier"].C_PRODUCTION_FRIGATEADVANCED = {
	"STATUS_CARRIER_AdvancedFrigateFacilityUnderAttack_1",
	"STATUS_CARRIER_AdvancedFrigateFacilityDamaged_1",
	"STATUS_CARRIER_AdvancedFrigateFacilityDestroyed_1",
}
ssMap["Carrier"].C_SENSORS_ADVANCEDARRAY = {
	"STATUS_CARRIER_SensorArrayUnderAttack_1",
	"STATUS_CARRIER_SensorArrayDamaged_1",
	"STATUS_CARRIER_SensorArrayDestroyed_1",
}

ssMap["Shipyard"].MS_MODULE_CLOAKGENERATOR = {
	"STATUS_SHIPYARD_CloakGeneratorUnderAttack_1",
	"STATUS_SHIPYARD_CloakGeneratorDamaged_1",
	"STATUS_SHIPYARD_CloakGeneratorDestroyed_1",
}
ssMap["Shipyard"].MS_MODULE_FIRECONTROL = {
	"STATUS_SHIPYARD_FireControlTowerUnderAttack_1",
	"STATUS_SHIPYARD_FireControlTowerDamaged_1",
	"STATUS_SHIPYARD_FireControlTowerDestroyed_1",
}
ssMap["Shipyard"].MS_MODULE_HYPERSPACE = {
	"STATUS_SHIPYARD_HyperspaceCoreUnderAttack_1",
	"STATUS_SHIPYARD_HyperspaceCoreDamaged_1",
	"STATUS_SHIPYARD_HyperspaceCoreDestroyed_1",
}
ssMap["Shipyard"].MS_MODULE_RESEARCH = {
	"STATUS_SHIPYARD_ResearchModuleUnderAttack_1",
	"STATUS_SHIPYARD_ResearchModuleDamaged_1",
	"STATUS_SHIPYARD_ResearchModuleDestroyed_1",
}
ssMap["Shipyard"].MS_MODULE_PLATFORMCONTROL = {
	"STATUS_SHIPYARD_PlatformFacilityUnderAttack_1",
	"STATUS_SHIPYARD_PlatformFacilityDamaged_1",
	"STATUS_SHIPYARD_PlatformFacilityDestroyed_1",
}
ssMap["Shipyard"].MS_PRODUCTION_CORVETTE = {
	"STATUS_SHIPYARD_CorvetteFacilityUnderAttack_1",
	"STATUS_SHIPYARD_CorvetteFacilityDamaged_1",
	"STATUS_SHIPYARD_CorvetteFacilityDestroyed_1",
}
ssMap["Shipyard"].MS_PRODUCTION_FIGHTER = {
	"STATUS_SHIPYARD_FighterFacilityUnderAttack_1",
	"STATUS_SHIPYARD_FighterFacilityDamaged_1",
	"STATUS_SHIPYARD_FighterFacilityDestroyed_1",
}
ssMap["Shipyard"].MS_PRODUCTION_FRIGATE = {
	"STATUS_SHIPYARD_FrigateFacilityUnderAttack_1",
	"STATUS_SHIPYARD_FrigateFacilityDamaged_1",
	"STATUS_SHIPYARD_FrigateFacilityDestroyed_1",
}
ssMap["Shipyard"].MS_PRODUCTION_FRIGATEADVANCED = {
	"STATUS_SHIPYARD_AdvancedFrigateFacilityUnderAttack_1",
	"STATUS_SHIPYARD_AdvancedFrigateFacilityDamaged_1",
	"STATUS_SHIPYARD_AdvancedFrigateFacilityDestroyed_1",
}
...
ssMap["Shipyard"].SY_PRODUCTION_CAPSHIP = {
	"STATUS_SHIPYARD_CapitalShipFacilityUnderAttack_1",
	"STATUS_SHIPYARD_CapitalShipFacilityDamaged_1",
	"STATUS_SHIPYARD_CapitalShipFacilityDestroyed_1",
}

ssMap["MotherShip"].MS_MODULE_CLOAKGENERATOR = {
	"STATUS_Mothership_CloakGeneratorUnderAttack_1",
	"STATUS_Mothership_CloakGeneratorDamaged_1",
	"STATUS_Mothership_CloakGeneratorDestroyed_1",
}
ssMap["MotherShip"].MS_MODULE_FIRECONTROL = {
	"STATUS_Mothership_FireControlTowerUnderAttack_1",
	"STATUS_Mothership_FireControlTowerDamaged_1",
	"STATUS_Mothership_FireControlTowerDestroyed_1",
}
ssMap["MotherShip"].MS_MODULE_HYPERSPACE = {
	"STATUS_Mothership_HyperspaceCoreUnderAttack_1",
	"STATUS_Mothership_HyperspaceCoreDamaged_1",
	"STATUS_Mothership_HyperspaceCoreDestroyed_1",
}
ssMap["MotherShip"].MS_MODULE_RESEARCH = {
	"STATUS_Mothership_ResearchModuleUnderAttack_1",
	"STATUS_Mothership_ResearchModuleDamaged_1",
	"STATUS_Mothership_ResearchModuleDestroyed_1",
}
ssMap["MotherShip"].MS_MODULE_PLATFORMCONTROL = {
	"STATUS_Mothership_PlatformFacilityUnderAttack_1",
	"STATUS_Mothership_CorvetteFacilityDamaged_1",
	"STATUS_Mothership_PlatformFacilityDestroyed_1",
}
ssMap["MotherShip"].MS_PRODUCTION_CORVETTE = {
	"STATUS_Mothership_CorvetteFacilityUnderAttack_1",
	"STATUS_SHIPYARD_CorvetteFacilityDamaged_1",
	"STATUS_Mothership_CorvetteFacilityDestroyed_1",
}
ssMap["MotherShip"].MS_PRODUCTION_FIGHTER = {
	"STATUS_Mothership_FighterFacilityUnderAttack_1",
	"STATUS_Mothership_FighterFacilityDamaged_1",
	"STATUS_Mothership_FighterFacilityDestroyed_1",
}
ssMap["MotherShip"].MS_PRODUCTION_FRIGATE = {
	"STATUS_Mothership_FrigateFacilityUnderAttack_1",
	"STATUS_SHIPYARD_FrigateFacilityDamaged_1",
	"STATUS_Mothership_FrigateFacilityDestroyed_1",
}
ssMap["MotherShip"].MS_PRODUCTION_FRIGATEADVANCED = {
	"STATUS_Mothership_AdvancedFrigateFacilityUnderAttack_1",
	"STATUS_Mothership_AdvancedFrigateFacilityDamaged_1",
	"STATUS_Mothership_AdvancedFrigateFacilityDestroyed_1",
}
ssMap["MotherShip"].MS_INNATE_ENGINE = {
	"STATUS_Mothership_EnginesUnderAttack_1",
	"STATUS_Mothership_EnginesDamaged_1",
	"STATUS_Mothership_EnginesDisabled_1",
}
ssMap["MotherShip"].MS_PRODUCTION_CAPSHIP = {
	"STATUS_Mothership_CapitalShipFacilityUnderAttack_1",
	"STATUS_Mothership_CapitalShipFacilityDamaged_1",
	"STATUS_Mothership_CapitalShipFacilityDestroyed_1",
}
ssMap["MotherShip"].MS_SENSORS_ADVANCEDARRAY = {
	"STATUS_Mothership_SensorArrayUnderAttack_1",
	"STATUS_Mothership_SensorArrayDamaged_1",
	"STATUS_Mothership_SensorArrayDestroyed_1",
}
ssMap["MotherShip"].MS_PRODUCTION_CAPSHIPADVANCED = {
	"STATUS_Mothership_AdvancedCapitalShipFacilityUnderAttack_1",
	"STATUS_Mothership_AdvancedCapitalShipFacilityDamaged_1",
	"STATUS_Mothership_AdvancedCapitalShipFacilityDestroyed_1",
}
...
function StatusSubsystem(ssname, shipname, event)
	...
	if
		(ssMap["" .. genericShipName] ~= nil)
		and (ssMap["" .. genericShipName]["" .. genericssname] ~= nil and event ~= Repaired)
	then
		if genericShipName == "Shipyard" then
			playSpeechActor(
				ssMap["" .. genericShipName]["" .. genericssname][event],
				NameSupportPilot,
				NumSupportPilots,
				Frequency_Status
			)
