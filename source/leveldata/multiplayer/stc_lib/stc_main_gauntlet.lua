...
-- MODIFICATION FOR INVASION GAME MODE
-- dofilepath("data:leveldata/multiplayer/stc_lib/stc_sobSplitter.lua")
dofilepath("data:leveldata/multiplayer/stc_lib/stc_sobSplitter_invasion.lua")
-- MODIFICATION FOR INVASION GAME MODE
	...
	for i = 0, Universe_PlayerCount()-1 do
		if(Player_IsAlive(i) == 1 or i == ALLIANCE_playerSlot) then
			for j = 1, getn(globalShipTable[i]) do
				...
				UPGRADE_Rule(i, j)
				experienceRule(i, j)
				-- Player ships should build fast. Enemy ships no build at all. 
				if i == 0 then
					FASTBUILD_Rule(i, j)
				else
					NOBUILD_Rule(i, j)
