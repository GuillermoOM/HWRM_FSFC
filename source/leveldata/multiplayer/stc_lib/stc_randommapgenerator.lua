dustCloudColourTable = {1.0, 1.0, 1.0, 1.0}
...
function STC_DetermChunk()
	...
	cluster_count = map_size*40/200000
	...
	while i <= total_so_far do
		...
		for k = 1, children_count do
			...
			-- create dustcloud
			if random() < 0.75 then
				addDustCloud("dustcloud", "dustcloud_0", {NewX[1], NewX[3], NewX[2]}, vmultiplyV(dustCloudColourTable, {1.00, 1.00, 1.00, 0.5*(1 + 0.5*rand())}), 0.0, 8500*(1+0.75*random()))
			end
	...
	-- add chance for some dangerous nebula
	...
	for i = 0, maxPlayers-1 do
		if random() < 0.5 then
			...
			for k = 1, floor(radius/5000) do
				...
				-- add some derelicts
				for m = 1, random(1, 5) do
					local y = vmultiply(randPoiss3(), radius)
					y = vaddV(x, y)
					
					if random() < 0.5 then
						addSalvage("Slv_Chunk_Sml0"..random(1, 8), {y[1], y[3], y[2]}, 0, 0, 0, 0, 0)
					else
						addSalvage("Slv_Chunk_Lrg0"..random(1, 5), {y[1], y[3], y[2]}, 0, 0, 0, 0, 0)
					end
				end
			end
		end
...
function STC_NonDetermChunk()
	...
	map_size = (30000 + 5000*rand())*maxPlayers
	...
	local n = random(1, 41-5)
	print("Background is "..backgroundTable[n])
	loadBackground(backgroundTable[n])
	
	
	setSensorsManagerCameraDistances(map_size/10, map_size)
	setDefaultMusic("Data:sound/music/ambient/amb_13")
end
	...
	while k <= iAsteroids and counter < 10 do
		...
		if vlength(y) < radius then
			...
			if goon == 1 then
				...
				if random() < 0.20 then
					addAsteroid("Asteroid_"..random(1, 2), {x[1], x[3], x[2]}, 100, 0, 0, 0, 0)
				else
					addAsteroid("Asteroid_"..random(3, 5), {x[1], x[3], x[2]}, 100, 0, 0, 0, 0)
				end
