...
function CpuBuild_PersonalityDemand()
	...
	aitrace("PersonalityDemand: Fi:" ..
		sg_classPersonalityDemand[eFighter] ..
		" Co:" .. sg_classPersonalityDemand[eCorvette] .. " Fr:" .. sg_classPersonalityDemand[eFrigate])
end
...
function DetermineSpecialDemand()
	local isFSRace = 0
	if SelfRace_GetNumber("cfg_race_index_sort", 1.0) >= 10.0 then
		isFSRace = 1
	end
		...
		if (SelfRace_GetNumber("cfg_buildable_subsystems", 1.0) >= 1.0) then
			...
			if (ShipDemandGet(kInterceptor) > 0 and NumSquadrons(kInterceptor) < 7 and s_militaryPop < 22) then
				if isFSRace then
					...
					if IsResearchDone(FIGHTERDRIVE) == 1 then
						ShipDemandAdd(kInterceptor, 0.5)
					end
				end
			end
	...
	if ((GetNumCollecting() > 9 or GetRU() > 1500) and s_militaryPop > 8 and UnderAttackThreat() < -75) then
		if isFSRace then
			...
			if (SelfRace_GetNumber("cfg_bc_can_upgrade", 0.0) >= 1.0) then
				...
				if kMissileDestroyer ~= nil then ShipDemandAdd(kMissileDestroyer, 0.65) end
				if (IsResearchDone(HEAVYGUNS) == 1) or isFSRace then
...
function DetermineScoutDemand()
	...
	if CPUPLAYERS_NORUSHTIME5 ~= nil or isFSRace then
